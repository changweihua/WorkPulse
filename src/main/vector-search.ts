...
function getDailyApiLimit(): number {
  try {
    ...
    const active = config.embeddingConfigs?.find(
      (e: any) => e.id === config.activeEmbeddingConfigId,
    );
...
function getActiveEmbeddingConfig(): EmbeddingConfig | null {
  try {
    ...
    return {
      id: active.id,
      provider: (active as any).provider || 'embedding',
...
function getDailyCallCount(): number {
  try {
    ...
    return modelGetDailyCallCount(activeId, getEmbeddingQuotaGroup());
  } catch {
    return 0;
  }
}
...
function incrementDailyCallCount(): number {
  try {
    ...
    modelIncrementDailyCallCount(activeId, group);
    return modelGetDailyCallCount(activeId, group);
  } catch {
    return 0;
  }
}
...
async function callEmbeddingAPI(texts: string[]): Promise<number[][]> {
  ...
  if (!resp.ok) {
    ...
    try {
      logAiUsage({
        ...
        success: false,
        error_msg: `HTTP ${resp.status}: ${body.slice(0, 200)}`,
      });
    } catch (err) {
      log.error('[VectorSearch] 记录 embedding 失败调用用量失败:', err);
    }
  ...
  try {
    logAiUsage({
      ...
      provider: config.provider,
      usage_type: 'embedding',
      input_tokens: data.usage?.prompt_tokens ?? texts.reduce((s, t) => s + t.length, 0),
      output_tokens: 0,
    ...
    log.info(
      `[VectorSearch] 已记录 embedding 用量: model=${config.model}, tokens=${data.usage?.total_tokens ?? 0}`,
    );
  } catch (err) {
    log.error('[VectorSearch] 记录 embedding 用量失败:', err);
  }
...
class VectorSearchService {
  ...
  async autoIndexAll(): Promise<{ indexed: number; errors: number; skipped: number }> {
    ...
    const existingHashes = db
      .prepare('SELECT worklog_id, content_hash FROM worklog_vectors')
      .all() as Array<{
      worklog_id: number;
      content_hash: string;
    }>;
    ...
    log.info(
      `[VectorSearch] 发现 ${toProcess.length} 条待向量化 worklog（剩余 API 额度: ${remaining}）`,
    );
    ...
    log.info(
      `[VectorSearch] 增量向量化完成: ${indexed} 成功, ${errors} 失败, ${unchangedIds.length} 内容未变`,
    );
  ...
  async search(
    ...
    // 先检查查询缓存
    const cachedRow = db
      .prepare('SELECT embedding FROM query_vectors WHERE query_text = ?')
      .get(query) as { embedding: string } | undefined;
  ...
  getStats(): VectorStats {
    this.initialize();
    const db = getDatabase();
    const count = db.prepare('SELECT COUNT(*) as cnt FROM worklog_vectors').get() as {
      cnt: number;
    };
  ...
  async indexSingleWorklog(id: number, content: string): Promise<boolean> {
    ...
    // 检查是否已有相同内容的向量
    const existing = db
      .prepare('SELECT content_hash FROM worklog_vectors WHERE worklog_id = ?')
