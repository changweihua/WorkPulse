  ...
  const loadMonthEvents = useCallback(async (year: number, month: number) => {
    ...
    try {
      ...
      for (const ev of list || []) {
        if (!marks[ev.event_date])
          marks[ev.event_date] = { todo: 0, done: 0, meeting: 0, events: [] };
        ...
        <div className="lg:w-2/5 xl:w-[420px] flex flex-col gap-4 min-h-0 overflow-y-auto">
          ...
          <div className="surface-card rounded-xl p-4 border-l-4 border-blue-500 shrink-0">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xs font-semibold text-blue-500 uppercase tracking-wider mb-0.5">
                  当前选中
                </h3>
            ...
            {showForm && (
              <Fade className="shrink-0 p-4 space-y-2.5 surface-inset border-b border-[var(--color-border)]">
                <input
                  ...
                  placeholder={
                    activeTab === 'todo' ? '待办内容，如：完成周报' : '会议主题，如：项目评审会'
                  }
                  autoFocus
              ...
              {eventsLoading ? (
                <div className="space-y-2 pt-2">
                  {Array.from({ length: 4 }, (_, i) => (
                    <div key={i} className="px-2.5 py-2 rounded-lg space-y-1.5">
                      <SkeletonLine width={`${55 + i * 7}%`} height="0.875rem" />
                      <SkeletonLine width={`${35 + i * 5}%`} height="0.625rem" />
                    </div>
                  ))}
                </div>
              ) : activeTab === 'todo' ? (
                    ...
                    {todos.map((t) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {t.description && (
                            <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5 break-words">
                              {t.description}
                            </p>
                          )}
                        </div>
                        <div className="shrink-0 hidden group-hover:flex items-center gap-0.5">
                          <IconBtn onClick={() => openEditForm(t)} title="编辑">
                            <Pencil size={12} />
                          </IconBtn>
                          <IconBtn onClick={() => handleDelete(t.id)} title="删除" danger>
                            <Trash2 size={12} />
                          </IconBtn>
                        </div>
                      </li>
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          {m.description && (
                            <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-1 break-words">
                              {m.description}
                            </p>
                          )}
                        </div>
                        <div className="shrink-0 hidden group-hover:flex items-center gap-0.5">
                          <IconBtn onClick={() => openEditForm(m)} title="编辑">
                            <Pencil size={12} />
                          </IconBtn>
                          <IconBtn onClick={() => handleDelete(m.id)} title="删除" danger>
                            <Trash2 size={12} />
                          </IconBtn>
                        </div>
                      </div>
