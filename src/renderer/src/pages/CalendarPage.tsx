...
const CalendarPage: React.FC = () => {
    ...
    return (
        <div className="h-full flex flex-col px-4 md:px-6 pt-6 md:pt-10 pb-6 md:pb-10 overflow-hidden">
            <div className="flex flex-1 flex-col lg:flex-row gap-6 w-full lg:w-[80vw] mx-auto min-h-0">
                ...
                <div ref={rightPanelRef} className="lg:w-1/3 xl:w-2/5 flex flex-col gap-4 flex-1 min-h-0">
                    {/* 固定区域 */}
                    <div ref={fixedPanelRef} className="surface-card p-5 border-l-4 border-blue-500 shrink-0">
                    ...
                    {/* 历史记录 */}
                        ...
                        <div className="flex items-center justify-between mb-3 shrink-0">
                            <h3 className="text-sm font-semibold text-zinc-700 dark:text-zinc-200 uppercase tracking-wider">点击历史</h3>
                            {records.length > 0 && (
                        ...
                        >
                            {records.length === 0 ? (
                                <p className="text-zinc-400 dark:text-zinc-500 text-sm text-center mt-6">暂无记录</p>
                            ) : (
                                <ul className="divide-y divide-zinc-100 dark:divide-zinc-800">
                                    {records.map((rec, idx) => (
                                        <li key={idx} className="py-2 flex justify-between text-sm">
                                            <span className="font-medium text-zinc-700 dark:text-zinc-200">{rec.dateStr}</span>
                                            <span className="text-zinc-500 dark:text-zinc-400">{rec.statusText}</span>
                                        </li>
                                    ))}
                                </ul>
                            )}
