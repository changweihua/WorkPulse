    ...
    <div className="shrink-0 w-[280px] h-full border-r border-[var(--color-border)] surface-card flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6">
        ...
        <div>
          ...
          <p
            className="mt-1.5 text-[10px] text-zinc-400 dark:text-zinc-500 truncate"
            title={currentModel}
          >
            {currentModel}
          </p>
            ...
            {progressItems.length > 0 && (
              <div className="mt-2 space-y-0.5">
                {progressItems.map((item) => (
                  <div
                    key={item.file}
                    className="flex justify-between text-[10px] text-emerald-500/80 dark:text-emerald-400/70"
                  >
        ...
        {isError && error && (
          <div className="p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200/70 dark:border-red-800/50">
            ...
            <p className="text-[11px] text-red-500/90 dark:text-red-400/80 mt-1 break-all">
              {error}
            </p>
...
function OcrPageContent() {
  ...
  return (
    <div className="h-full overflow-hidden flex gap-4 px-6 py-4">
      <ModelPanel
        status={status}
      ...
      {/* 右侧主区 */}
      <div className="flex-1 flex gap-4 flex-col min-w-0 h-full">
        {/* 顶栏 */}
        ...
        <div className="flex-1 overflow-y-auto p-0 flex flex-col">
          <div className="mx-auto w-full flex-1 flex flex-col gap-6 surface-card rounded-xl p-6">
            {/* 图片上传区 */}
            {!imageUrl ? (
              <div
                ...
                onDrop={handleDrop}
                className={`flex flex-col items-center justify-center py-14 rounded-xl border-2 border-dashed cursor-pointer transition-all flex-1 ${
                  dragOver
                    ? 'border-emerald-400 scale-[1.01]'
                    : 'border-zinc-300 dark:border-zinc-700 hover:border-emerald-300 dark:hover:border-emerald-700'
                }`}
              >
                <div
              ...
              <div className="relative surface-card rounded-xl p-3 group">
                ...
                <div className="flex items-center justify-between mt-2.5 px-1">
                  <span
                    className="text-xs text-zinc-400 dark:text-zinc-500 truncate max-w-[70%]"
