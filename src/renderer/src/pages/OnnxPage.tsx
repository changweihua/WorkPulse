            ...
            {progressItems.length > 0 && (
              <div className="mt-2 space-y-0.5">
                {progressItems.map((item) => (
                  <div
                    key={item.file}
                    className="flex justify-between text-[10px] text-violet-500/80 dark:text-violet-400/70"
                  >
...
function OnnxPageContent() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === assistantId ? { ...m, content: `❌ 推理失败：${(err as Error).message}` } : m,
        ),
      );
      try {
        window.api.aiUsage.log({
  ...
  return (
    <div className="h-full overflow-hidden flex gap-4 px-8 py-4">
      <ModelPanel
        status={status}
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full">
        ...
        <div className="flex-1 overflow-y-auto  py-4 flex flex-col gap-4">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center flex-1 text-center surface-card rounded-xl p-8">
              ...
              <h2 className="text-lg font-semibold text-zinc-800 dark:text-zinc-100 mb-1.5">
                本地 AI 推理
              </h2>
                ...
                >
                  ...
                  <div className={`max-w-[78%] ${isUser ? '' : 'flex-1 min-w-0'}`}>
                    {isUser ? (
                      ...
                      <ResultBlock
                        content={msg.content}
                        cursor={streamingId === msg.id && isGenerating}
                      />
                    ) : (
