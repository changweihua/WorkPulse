...
function ReportPage(): ReactNode {
  ...
  const handleGenerate = async (): Promise<void> => {
    ...
    const logsText = logs
      .map((log: { created_at: string; content: string }) => `[${log.created_at}] ${log.content}`)
      .join('\n');
    const taskText =
  ...
  return (
    <div className="h-full overflow-hidden px-6 py-4">
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5" style={{ height: '100%' }}>
        <div
          className="lg:col-span-3 min-w-0 flex flex-col gap-3 overflow-hidden"
          style={{ height: '100%' }}
        >
          {isViewingHistory && (
          ...
          {status === 'no_data' && !isViewingHistory && (
            <div className="text-center py-10 surface-card rounded-xl shrink-0">
              ...
              <p className="text-zinc-600 dark:text-zinc-400 font-medium mb-1">
                {t('report.noDataTitle')}
              </p>
              <p className="text-zinc-400 dark:text-zinc-500 text-sm">
                {t('report.noDataSubtitle')}
              </p>
            </div>
          )}
          ...
          {(status === 'success' || status === 'streaming' || status === 'generating') && (
            <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              {editing ? (
                ...
                <div
                  className={`flex-1 min-h-0 border border-zinc-200 dark:border-zinc-700 rounded-xl p-5 surface-card ${isStreaming ? 'streaming-border' : ''}`}
                  style={{ overflow: 'auto' }}
                >
                  <StreamedMarkdown
          ...
          {status === 'idle' && !isViewingHistory && !reportContent && (
            <div className="hidden lg:flex flex-1 flex-col items-center justify-center surface-card rounded-xl p-8">
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6 leading-relaxed text-center max-w-md">
                {t('report.emptyDesc')}
              </p>
        ...
        <div
          className="lg:col-span-2 min-w-0 overflow-hidden flex flex-col"
          style={{ height: '100%' }}
        >
                ...
                {history.map((report) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <FileText
                          className={`w-4 h-4 shrink-0 ${isActive ? 'text-zinc-700 dark:text-zinc-200' : 'text-zinc-400'}`}
                        />
