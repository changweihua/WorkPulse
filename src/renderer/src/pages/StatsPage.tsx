...
import { SkeletonLine, SkeletonRect, SkeletonStatCard } from '../components/Skeleton';
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        ...
        <p
          className="text-[32px] font-extrabold tabular-nums leading-none tracking-tight"
          style={{ color: valueColor }}
        >
          {displayValue}
          {suffix && (
            <span className="text-sm font-semibold text-zinc-400 dark:text-zinc-500 ml-0.5">
              {suffix}
            </span>
          )}
        </p>
        {delta && (
          <p className="text-[11px] text-zinc-400 dark:text-zinc-500 mt-1.5">
            ...
            <span
              className={`font-medium ${delta.startsWith('+') ? 'text-emerald-500' : 'text-red-500'}`}
            >
        ...
        series: [
          {
            ...
            label: {
              show: true,
              position: 'center',
              formatter: [`{total|${total}}`, `{label|${tRef.current('stats.total')}}`].join('\n'),
              rich: {
                total: {
                  fontSize: 24,
                  fontWeight: 'bold',
            ...
            data: [
              ...
              {
                value: tasks,
                name: tRef.current('stats.doneTasks'),
  ...
  return (
    <div className="flex flex-col items-center">
      {enabled ? (
        ...
        <div className="w-40 h-40 animate-pulse bg-zinc-100 dark:bg-zinc-800 rounded" />
      )}
...
function WeeklySummary({ data }: { data: DailyStats[] }): ReactNode {
  ...
  const bestDay = periodData.reduce(
    (best, d) =>
      d.log_count + d.task_completed > best.count
        ? { date: d.date, count: d.log_count + d.task_completed }
        : best,
    { date: '', count: 0 },
  );
  const avgPerActiveDay =
    activeDays > 0 ? ((periodLogs + periodTasks) / activeDays).toFixed(1) : '0';
  ...
  const rows = [
    ...
    { label: t('stats.doneTasks'), value: String(periodTasks) },
    {
      ...
      value: bestDay.count > 0 ? `${bestDay.date.slice(5)} · ${bestDay.count}` : '-',
    },
    ...
    <FadeIn className="surface-card rounded-xl p-5" delay={0.12}>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {rows.map((row, idx) => (
          ...
          >
            <p
              className={`text-[11px] text-zinc-500 dark:text-zinc-400 ${idx === 3 ? '' : 'mb-1'}`}
            >
              {row.label}
            </p>
            <p
  ...
  return (
    <FadeIn className="surface-card rounded-xl p-5 relative" delay={0.24}>
      ...
      {enabled ? (
        ...
        <div className="h-52 w-full animate-pulse bg-zinc-100 dark:bg-zinc-800 rounded" />
      )}
      {loading && (
        <div className="flex items-center justify-center py-8 text-zinc-400 text-xs absolute inset-0">
          {t('common.loading')}
        </div>
      )}
      {!loading && !hasData && (
...
function AISummary({ stats }: { stats: Stats }): ReactNode {
  ...
  const lines = [
    ...
    bestDay.count > 0
      ? t('stats.aiBestDay', { day: bestDay.date.slice(5), count: bestDay.count })
      : null,
...
function computeDeltas(daily: DailyStats[]): {
  logsDelta: string | undefined;
  tasksDelta: string | undefined;
} {
        ...
        <div className="hide-scrollbar h-full overflow-y-auto px-6 py-4 space-y-5">
          <FadeIn>
            <div>
              ...
              <SkeletonLine width="12rem" height="0.875rem" className="mt-2" />
            </div>
          </FadeIn>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
            <SkeletonRect className="lg:col-span-3 surface-card rounded-xl" height="18rem" />
            <div className="lg:col-span-2 space-y-5">
              <SkeletonRect className="surface-card rounded-xl" height="12rem" />
              <SkeletonRect className="surface-card rounded-xl" height="10rem" />
            </div>
          </div>
          <SkeletonRect className="surface-card rounded-xl" height="16rem" />
          <SkeletonRect className="surface-card rounded-xl" height="14rem" />
        </div>
      </div>
    );
  }
