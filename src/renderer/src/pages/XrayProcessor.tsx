...
const FullscreenEditor: React.FC<FullscreenEditorProps> = ({
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    for (let i = materials.length - 1; i >= 0; i--) {
      ...
      if (
        canvasX >= mat.x &&
        canvasX <= mat.x + mat.width &&
        canvasY >= mat.y &&
        canvasY <= mat.y + mat.height
      ) {
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-95 flex flex-col">
      ...
      <div className="flex justify-between items-center p-4 bg-zinc-900 text-white">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleCancel} className="px-4 py-2 bg-red-600 hover:bg-red-700 rounded">
            ❌ 取消
          </button>
        </div>
      </div>
      ...
      {/* 工具栏 */}
      <div className="flex flex-wrap items-center gap-2 p-2 bg-zinc-800 text-white">
        <input
          type="file"
          accept="image/*"
        ...
        />
        <label
          htmlFor="materialInputFullscreen"
          className="px-3 py-1 bg-blue-600 rounded cursor-pointer hover:bg-blue-700"
        >
          + 添加素材
        </label>
        <button onClick={zoomIn} className="px-3 py-1 bg-zinc-600 rounded hover:bg-zinc-700">
          🔍+
        </button>
        <button onClick={zoomOut} className="px-3 py-1 bg-zinc-600 rounded hover:bg-zinc-700">
          🔍-
        </button>
        <button onClick={fitToScreen} className="px-3 py-1 bg-zinc-600 rounded hover:bg-zinc-700">
          适应
        </button>
        <span className="text-sm text-zinc-400 dark:text-zinc-500 ml-4">滚轮缩放 · 拖拽素材</span>
      </div>

      {/* 素材缩略图列表 */}
      <div className="flex gap-2 p-2 bg-zinc-800 border-t border-zinc-700 overflow-x-auto">
        {materials.map((mat) => (
          <div
            key={mat.id}
            className={`relative border-2 p-1 cursor-pointer ${selectedId === mat.id ? 'border-green-500' : 'border-zinc-600'}`}
        ...
        {materials.length === 0 && (
          <span className="text-zinc-400 dark:text-zinc-500 text-sm">暂无素材</span>
        )}
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-4 bg-black">
        <canvas
          ref={canvasRef}
          className="max-w-full max-h-full border border-zinc-600"
    ...
    <div className="flex flex-col lg:flex-row gap-8 px-6 py-4 h-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col surface-card p-4 rounded-xl shadow-md min-h-0 min-w-0 overflow-hidden">
        <div className="flex justify-between items-center mb-3 shrink-0">
          <h3 className="text-lg font-bold text-zinc-800 dark:text-zinc-100">📷 原图</h3>
          <button
        ...
        <div className="flex-1 min-h-0 border border-zinc-200 dark:border-zinc-600/70 rounded-lg overflow-hidden bg-black flex items-center justify-center">
          <canvas ref={previewCanvasRef} className="max-w-full max-h-full block" />
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2 justify-center shrink-0">
          ...
          <label
            htmlFor="fileInput"
            className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg cursor-pointer hover:bg-blue-700 transition shadow-sm"
          >
            📁 选择原图
          </label>
          <button
        ...
        {/* 全局参数 - 紧凑行布局 */}
        <div className="mt-2 pt-2 border-t border-zinc-200 dark:border-zinc-700/70 flex gap-4 shrink-0">
          <label className="flex-1 text-xs">
            <span className="flex justify-between text-zinc-500 dark:text-zinc-300">
              饱和度 <span className="font-mono">{Math.round(saturationBoost * 100)}%</span>
            </span>
            <input
              type="range"
              min="0"
          ...
          <label className="flex-1 text-xs">
            <span className="flex justify-between text-zinc-500 dark:text-zinc-300">
              细节 <span className="font-mono">{Math.round(detailBoost * 100)}%</span>
            </span>
            <input
              type="range"
              min="0"
      ...
      <div className="flex-1 flex flex-col surface-card p-4 rounded-xl shadow-md min-w-0 min-h-0 overflow-hidden">
        <div className="flex-1 flex flex-col items-center min-h-0 overflow-y-auto">
          <div className="flex flex-col items-center mb-4 w-full">
            <h4 className="text-sm font-semibold text-zinc-600 dark:text-zinc-200 mb-1">☀️ 加亮</h4>
            <div
              className="w-full max-w-md border border-zinc-200 dark:border-zinc-600/70 rounded-lg overflow-hidden bg-black cursor-pointer relative"
              onClick={() => {
            ...
            >
              ...
              <div className="absolute bottom-1 right-1 text-white bg-black bg-opacity-50 px-2 py-0.5 text-xs rounded">
                点击全屏
              </div>
            </div>
            <div className="w-full max-w-md mt-1.5">
              <label className="block text-xs">
                <span className="flex justify-between text-zinc-500 dark:text-zinc-300">
                  加亮强度 <span className="font-mono">{Math.round(brightStrength * 100)}%</span>
                </span>
                <input
                  type="range"
                  min="0"
          ...
          <div className="flex flex-col items-center w-full">
            <h4 className="text-sm font-semibold text-zinc-600 dark:text-zinc-200 mb-1">🌙 加暗</h4>
            <div
              className="w-full max-w-md border border-zinc-200 dark:border-zinc-600/70 rounded-lg overflow-hidden bg-black cursor-pointer relative"
              onClick={() => {
            ...
            <div className="w-full max-w-md mt-1.5">
              <label className="block text-xs">
                <span className="flex justify-between text-zinc-500 dark:text-zinc-300">
                  加暗强度 <span className="font-mono">{Math.round(darkStrength * 100)}%</span>
                </span>
                <input
                  type="range"
                  min="0"
          ...
          <div className="mt-2 text-center text-xs text-zinc-400 dark:text-zinc-500 border-t border-zinc-200 dark:border-zinc-700/70 pt-2 w-full">
...
export default function XrayProcessor() {
  return (
    <ErrorBoundary>
      <Suspense
        fallback={
          <div className="p-10 text-center text-zinc-500 dark:text-zinc-300">
            ⏳ 加载 WASM 模块...
          </div>
        }
      >
