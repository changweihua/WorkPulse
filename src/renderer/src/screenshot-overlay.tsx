...
function ScreenshotOverlay(): React.ReactNode {
  ...
  // 展示提示：autoClose=true 时 2 秒后关闭覆盖窗口；否则仅短暂提示（占位功能）
  const showToastMsg = (msg: string, autoClose: boolean) => {
    if (toastTimerRef.current) clearTimeout(toastTimerRef.current)
    setToastMsg(msg)
    setToast(true)
    toastTimerRef.current = window.setTimeout(() => {
      setToast(false)
      if (autoClose) window.screenshotOverlayApi.cancel()
    }, autoClose ? 2000 : 1600)
  }
  ...
  const runAction = async (action: ToolAction) => {
    ...
    let ok = false
    try {
      const res = await window.screenshotOverlayApi.crop(
